...
public class EconomyStorageBackendFlatfile extends EconomyStorageBackendCaching {
    ...
    public void reloadDatabase() {
        ...
        try {
            ...
            if (schemaVer != SCHEMA_VERSION) {
                // ???
                Econo.logger().severe("Unrecognized flatfile database version " + schemaVer + ", cannot load database!");
                return;
            }
            ...
            Econo.logger().severe("Failed to load flatfile database!");
            e.printStackTrace();
        }
    }

    private void loadSchemaVersion1(File file) {
        Econo.logger().info("Upgrading flatfile database from version 1.");
        try {
            Files.copy(file, new File(file.getParentFile(), file.getName() + "-backup"));
            Econo.logger().info("Backed up old flatfile database.");
        } catch (IOException e) {
        ...
        try {
            ...
            Econo.logger().severe("Failed to upgrade flatfile database! Recommend reporting this bug and reverting to an older version of the plugin.");
    ...
    private void saveDatabase() {
        ...
        try {
            ...
            Econo.logger().severe("Failed to save flatfile database!");
        }
    }
